<template>
  <div class="account__details" v-if="user">
    <div class="account_detail">
      ...
      <q-form class="flex flex-center row form account-form bg-white" ref="updateBirthdayForm" v-if="forms.birthday.show"
        @submit.prevent="() => {
            handleUpdateAccount({
          ...
          ">
        <q-input outlined v-model="forms.birthday.value" autofocus lazy-rules :rules="[
          (val) =>
        ...
        ]" class="account-input bg-white" hide-bottom-space>
    ...
    <div class="account_detail">
      <div class="account_detail__label text-h6 flex items-center">
        ...
        <q-icon name="edit" color="secondary" class="cickable" @click="() => {
            forms.email.show = true
            forms.email.value = user.email
          }
          "></q-icon>
      </div>
      <q-form class="flex flex-center row form account-form" ref="updateEmailForm" v-if="forms.email.show"
        @submit.prevent="() => {
          ...
          ">
        <q-input name="email" outlined autofocus class="account-input bg-white" type="text" v-model="forms.email.value"
          lazy-rules :rules="[
            (val, rules) =>
              rules.email(val) ||
    ...
    <div class="account_detail">
      ...
      <q-form class="flex flex-center column form account-form account-password-form" ref="updateEmailForm"
        v-if="forms.password.show" @submit.prevent="() => {
          ...
          ">
        <q-input name="password" outlined label="Ancien mot de passe" class="q-mb-md account-input bg-white"
          :type="forms.password.showCurrentPassword ? 'text' : 'password'" v-model="forms.password.current" lazy-rules
          autofocus :rules="[
            (val) =>
              val.trim().length > 0 || 'Veullez remplir ce champ'
          ]" hide-bottom-space>
          <template v-slot:append>
            <q-icon :name="forms.password.showCurrentPassword ? 'visibility' : 'visibility_off'" class="cursor-pointer"
              color="secondary" @click="forms.password.showCurrentPassword = !forms.password.showCurrentPassword" />
          </template>
        </q-input>
        <q-input name="password" outlined label="Nouveau mot de passe" class="q-mb-md account-input bg-white"
          :type="forms.password.showNewPassword ? 'text' : 'password'" v-model="forms.password.new" lazy-rules
          ...
          <template v-slot:append>
            <q-icon :name="forms.password.showNewPassword ? 'visibility' : 'visibility_off'" class="cursor-pointer"
              color="secondary" @click="forms.password.showNewPassword = !forms.password.showNewPassword" />
          </template>
        </q-input>
        <q-input name="confirmPassword" outlined label="Confirmation du nouveau mot de passe"
          class="q-mb-md account-input bg-white" :type="forms.password.showConfirmPassword ? 'text' : 'password'"
          v-model="forms.password.confirm" lazy-rules :rules="[
            (val) =>
              val.trim().length > 0 || 'Veullez remplir ce champ',
            (val) =>
          ...
          <template v-slot:append>
            <q-icon :name="forms.password.showConfirmPassword ? 'visibility' : 'visibility_off'
  ...
  <div class="account__toggles-container flex column" v-if="user">
    <!-- <q-toggle
      ...
      "
    /> -->
    <q-toggle v-model="user.newsletter" label="Recevoir la newsletter" color="primary" class="q-mb-md"
      @update:model-value="
        handleUpdateAccount({ newsletter: user.newsletter })
        " />
  </div>
  <div class="account__btns-container">
    <q-btn label="Supprimer le compte" type="button" text-color="secondary" color="white" rounded
      @click.prevent="handleDeleteAccount()" :loading="deleteLoading" size="20px" padding="xs"
    ...
    <q-btn label="Déconnexion" type="button" color="secondary" rounded @click.prevent="handleLogout()"
      :loading="logoutLoading" size="20px" padding="xs" class="text-bold btn btn-secondary btn-full-width btn-thin" />
  </div>
</template>
